...
export function SubscriptionPlans() {
    ...
    return (
        <div className="flex flex-col h-full bg-main-app p-6 font-sans gap-8 overflow-y-auto">
            {/* Header matches Reference */}
            <div>
                <h1 className="text-2xl font-bold mb-1" style={{ color: 'var(--text-main)' }}>Pricing</h1>
                <div className="text-sm text-muted">Pages / <span>Pricing</span></div>
            </div>

            {/* Toggle Centered */}
            <div className="flex justify-center items-center gap-3">
                <span className={`text-sm font-medium ${billingCycle === 'monthly' ? 'text-gray-900 dark:text-white' : 'text-muted'}`}>Monthly</span>
                <button
                ...
                <span className={`text-sm font-medium ${billingCycle === 'yearly' ? 'text-gray-900 dark:text-white' : 'text-muted'}`}>Yearly</span>
            </div>

            {/* Plans Grid */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {plans.map((plan, idx) => (
                    <div key={idx} className="bg-surface rounded-xl border p-1 flex flex-col h-full relative group transition-all hover:shadow-lg" style={{ borderColor: 'var(--border-color)' }}>
                        {/* Card Body inner container match Ref */}
                        <div className="bg-surface rounded-lg p-6 flex flex-col h-full">
                            ...
                            <div className="mb-6">
                                ...
                                <div className="flex items-baseline gap-1">
                                    ...
                                    <span className="text-sm text-muted">/{billingCycle === 'monthly' ? 'monthly' : 'yearly'}</span>
                                </div>
                            </div>
                            ...
                            <div className="mb-6 flex-1 space-y-4">
                                <p className="text-sm font-bold text-muted uppercase tracking-wide mb-4">Features Includes</p>
                                {plan.features.map((feat, fIdx) => (
                                    <div key={fIdx} className="flex items-center gap-3 text-sm">
                                        ...
                                        <span className={feat.included ? 'text-main' : 'text-muted'}>{feat.text}</span>
                                    </div>
                                ))}
                            </div>
