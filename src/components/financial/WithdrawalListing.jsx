...
export function WithdrawalListing() {
    const [withdrawals, setWithdrawals] = useState([]);
    const [loading, setLoading] = useState(false);
    const [categories, setCategories] = useState([]);

    // Add Withdrawal Flow State
    const [isAddFlow, setIsAddFlow] = useState(false);
    const [selectedDate, setSelectedDate] = useState(new Date().toISOString().split('T')[0]);
    const [entries, setEntries] = useState([
        { id: 1, title: '', amount: '', explanation: '', paidTo: '', paidFrom: 'Cash' }
    ]);
    ...
    const fetchWithdrawals = async () => {
        try {
            setLoading(true);
            const res = await financialService.getWithdrawals();
    ...
    const fetchCategories = async () => {
        try {
            const res = await financialService.getWithdrawalCategories();
    ...
    const handleAddRow = () => {
        setEntries([...entries, { id: Date.now(), title: '', amount: '', explanation: '', paidTo: '', paidFrom: 'Cash' }]);
    };

    const handleClearRow = (id) => {
        setEntries(entries.map(e => e.id === id ? { ...e, title: '', amount: '', explanation: '', paidTo: '', paidFrom: 'Cash' } : e));
    };
    ...
    const handleSave = async () => {
        ...
        if (validEntries.length === 0) {
            toast.error('Please add at least one valid withdrawal (Title and Amount required)');
            return;
        }

        try {
            // API expects single object or array? Controller handles single create.
            // Let's loop for now or update controller to bulk create.
            // The controller `createWithdrawal` handles single object.
            // I should probably update controller to handle bulk or loop here.
            // For safety, I'll loop here.
            for (const entry of validEntries) {
                await financialService.createWithdrawal({
                    date: selectedDate,
                    title: entry.title,
            ...
            toast.success('Withdrawals saved successfully');
            setIsAddFlow(false);
            setEntries([{ id: 1, title: '', amount: '', explanation: '', paidTo: '', paidFrom: 'Cash' }]);
            fetchWithdrawals();
        } catch (error) {
            console.error('Error saving withdrawals:', error);
            toast.error('Failed to save withdrawals');
        }
    };

    // Chart Data
    const chartData = withdrawals.slice(0, 10).map(e => ({
        name: e.title,
        ...
        return (
            <div className="flex flex-col h-full bg-gray-50 font-sans p-6 gap-6 overflow-hidden w-full relative">
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <h1 className="text-2xl font-bold text-gray-800">Add Withdrawals</h1>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-gray-50 text-xs font-bold text-gray-800 border-b">
                            <tr>
                                ...
                                <th className="p-4 w-1/6">Paid To</th>
                                ...
                                <tr key={entry.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-2">
                                        <input
                                            ...
                                            placeholder="Paid To"
                                            value={entry.paidTo}
                                            onChange={(e) => handleEntryChange(entry.id, 'paidTo', e.target.value)}
                                        />
                                    </td>
                                    <td className="p-2">
                                        <select
                ...
                <div className="flex justify-end gap-4 mt-4">
                    ...
                    <button onClick={handleSave} className="px-6 py-2 bg-red-700 text-white rounded font-bold hover:bg-red-800 shadow-sm">Save Changes</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col h-full gap-4">
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm border flex flex-wrap items-end gap-4">
                ...
                <button onClick={() => setIsAddFlow(true)} className="px-6 py-2 bg-red-600 text-white rounded font-bold hover:bg-red-700 text-sm ml-auto">Add Withdrawal</button>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border min-h-[200px] flex flex-col justify-center">
                <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                    ...
                    Top 10 Withdrawals(View Chart) <ChevronDown className="w-4 h-4" />
                </h3>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-blue-50/50 text-xs font-bold text-gray-800 border-b">
                            <tr>
                                ...
                                <th className="p-4">Total Withdrawal (₹)</th>
                                <th className="p-4 text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="text-sm divide-y">
                            {withdrawals.length === 0 ? (
                                <tr><td colSpan="3" className="p-4 text-center text-gray-500">No withdrawals found</td></tr>
                            ) : (
            ...
            <div className="text-xs font-bold text-gray-500">
                Showing 1 to {withdrawals.length} of {withdrawals.length} records
            </div>
        </div>
    );
}
