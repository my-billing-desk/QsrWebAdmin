...
export function WelcomeBanner() {
    ...
    return (
        <div className="bg-white rounded-xl p-6 shadow-sm border flex items-center justify-between mb-6" style={{ borderColor: 'var(--border-color)' }}>
            <div className="flex items-center gap-5">
                {/* Avatar */}
                <div className="w-16 h-16 rounded-full flex items-center justify-center border-4 shrink-0" style={{ backgroundColor: 'var(--sidebar-active)', borderColor: 'var(--bg-gray-50)' }}>
                    <span className="text-2xl font-bold text-indigo-600">{user?.name?.charAt(0) || 'A'}</span>
            ...
            <div className="flex items-center gap-3">
                <button
                    ...
                    style={{ backgroundColor: 'var(--text-main)', color: 'var(--bg-white)', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}
                >
                    <Plus className="w-4 h-4" /> Add Project
                </button>
                <button
                    className="flex items-center gap-2 px-4 py-2 rounded-lg transition-colors text-sm font-semibold shadow-lg hover-bg-indigo-600"
