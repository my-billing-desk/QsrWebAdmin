...
export function TaxesView() {
    ...
    const columns = [
        {
            ...
            render: (item) => (
                <div className="flex items-center gap-2 font-medium text-main">
                    {item.name}
                    <Info className="w-3 H-3 text-muted opacity-50 cursor-help" />
                </div>
            )
        },
        {
    ...
    const headerControls = (
        <div className="flex items-center gap-2">
            <button className="flex items-center gap-2 px-3 py-1.5 border rounded text-sm font-medium bg-surface text-main hover:bg-main-app" style={{ borderColor: 'var(--border-color)' }}>
                <Calendar className="w-4 h-4 text-muted" />
                <span>29/12/2025 - 29/12/2025</span>
            </button>
            <button className="flex items-center gap-2 px-3 py-1.5 border rounded text-sm font-medium bg-surface text-main hover:bg-main-app" style={{ borderColor: 'var(--border-color)' }}>
                <span>Taxes List</span>
                <svg className="w-4 h-4 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
            </button>
            <button className="flex items-center gap-2 px-3 py-1.5 border rounded text-sm font-medium bg-surface text-main hover:bg-main-app" style={{ borderColor: 'var(--border-color)' }}>
    ...
    const actionButtons = (
        <div className="flex gap-2">
            <button className="flex items-center gap-2 px-4 py-2 border rounded-lg bg-surface hover:bg-main-app text-sm font-medium text-main shadow-sm transition-colors" style={{ borderColor: 'var(--border-color)' }}>
    ...
    return (
        <div className="flex flex-col h-full bg-main-app p-6 font-sans gap-6 overflow-hidden">

            {/* 1. Header & Breadcrumbs */}
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold mb-1" style={{ color: 'var(--text-main)' }}>Taxes</h1>
                    <div className="text-sm text-muted">HR / <span>Taxes</span></div>
                </div>
                {actionButtons}
            </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-surface w-full max-w-md rounded-xl shadow-2xl p-6 relative" style={{ backgroundColor: 'var(--bg-surface)' }}>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                ...
                                <input
                                    type="text"
                                    required
                                    className="w-full border rounded-lg px-3 py-2 bg-main-app focus:ring-1 focus:ring-primary outline-none"
                            ...
                            <div className="flex justify-center pt-4">
                                <button type="submit" className="w-full bg-primary text-white font-bold py-2.5 rounded-lg hover:opacity-90 transition-opacity" style={{ backgroundColor: 'var(--color-primary)' }}>
